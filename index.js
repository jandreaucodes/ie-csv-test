...
(async function () {
  ...
  for (const [index, chunk] of chunks.entries()) {
    ...
    for (const [i, res] of responses.entries()) {
      if (!res) {
        console.log(
          `${chalk.red("Error")}: Failed webhook for row ${chalk.yellow(
            index * CONCURRENCY + i + 1
          )}`
        );
        count--;
      }
    ...
    console.log(
      `Uploaded: Chunk ${chalk.yellow(index + 1)} (${chalk.yellow(count)} rows)`
    );
    await delay(DELAY);
  }
  ...
  async function sendRequest(payload) {
    const body =
      SIMULATE_CSV_UPLOADER === "true" ? { upload: payload } : payload;
    const params = {
      url: HOOK_URL,
      method: "POST",
      json: true,
  ...
  function readCsv() {
    ...
    function readFile() {
      try {
        ...
        console.log(
          `${chalk.red(
            "Error"
          )}: Could not find CSV. The file must be named "data.csv" and be located in this folder`
        );
        process.exit(1);
      }
    }
    ...
    function parseSheet(file) {
      try {
        console.log(csv.parse(file, { columns: true }));
